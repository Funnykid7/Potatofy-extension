...
export function computeSavings(counters, calibration = null, heapMeasurements = null) {
  ...
  if (hm && typeof hm === 'object') {
    ...
    const features = ['animationsKilled', 'videosPaused', 'videosPreloadNoned', 'autoplayKilled', 'imagesLazied', 'siteKillerHits'];

    for (const feature of features) {
      if (hm[feature] && Array.isArray(hm[feature]) && hm[feature].length > 0) {
        const measurements = hm[feature]
          .map(m => m.freed)
  ...
  let estimatedRamBytes =
    ...
    (c.videosPaused             || 0) * (heapMultipliers.videosPaused || w.videoPause.ramBytes) +
  ...
  // When realRamFreedBytes > 0 the tab-discard heuristic is removed from
  // totalRamBytes, so the breakdown must reflect the same adjustment to ensure
  // real + measured + estimated == totalRamBytes.
  // Invariant: heapMeasuredRamBytes is the sum of (counter × heapMultiplier) for features
  // that have real measurements; estimatedRamBytes already uses those same heapMultipliers
  // for the same features. Therefore (estimatedRamBytes - heapMeasuredRamBytes) is always
  // >= 0 and Math.max(0,...) is a safety net, not expected to fire. If it does fire, the
  // breakdown will sum to less than totalRamBytes — the tests.js 'breakdown parts sum' test
  // catches this regression.
  const tabDiscardHeuristic = (c.tabsDiscarded || 0) * w.tabDiscard.ramBytes;
  const estimatedBreakdownBytes = realRamFreedBytes > 0
    ? Math.max(0, estimatedRamBytes - tabDiscardHeuristic - heapMeasuredRamBytes)
    : Math.max(0, estimatedRamBytes - heapMeasuredRamBytes);
