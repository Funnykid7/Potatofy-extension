...
export function computeSavings(counters, calibration = null) {
  ...
  if (realRamFreedBytes > 0) {
    // Real measurement replaces the tabDiscard heuristic, but never below the
    // heuristic itself — some discards in the same counter window may have
    // failed to produce a real measurement (only incrementing tabsDiscarded,
    // not realRamFreed), so subtracting the FULL heuristic for every discard
    // understated the total whenever only some of them had real numbers.
    const tabDiscardSavings = Math.max(realRamFreedBytes, tabDiscardHeuristic);
